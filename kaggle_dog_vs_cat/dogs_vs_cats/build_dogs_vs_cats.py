...
# perform another stratified sampling , this time to build
# the validation data
split = train_test_split(trainPaths, trainLabels,
...
for (dType, paths, labels, outputPath) in datasets:
    ...
    for (i, (path, label)) in enumerate(zip(paths, labels)):
        # load the iamge and preprocess it
        image = cv2.imread(path)
        image = aap.preprocess(image)

        # if we are building the training dataset, the compute the 
        # mean of each chanel in the iamge, then update the respective lists
